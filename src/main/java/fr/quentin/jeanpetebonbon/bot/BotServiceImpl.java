...
import fr.quentin.jeanpetebonbon.bot.enums.BotCommand;
import fr.quentin.jeanpetebonbon.bot.guild.Guild;
import fr.quentin.jeanpetebonbon.bot.guild.GuildRepository;
import fr.quentin.jeanpetebonbon.core.exception.DateTimeBeforeException;
import fr.quentin.jeanpetebonbon.core.exception.NoServerFound;
import fr.quentin.jeanpetebonbon.core.exception.NoTextChannelException;
import fr.quentin.jeanpetebonbon.core.utils.RandomUtils;
...
public class BotServiceImpl extends ListenerAdapter implements BotService {
  ...
  PriorityQueue<ScheduledEventDto> queue =
      new PriorityQueue<>(Comparator.comparing(ScheduledEventDto::executeAt));
...
//          .get(event.getGuild().getIdLong()).getValue().getDelay(TimeUnit.SECONDS))).queue();
  ...
  private ReplyCallbackAction setEvent(String message, String date, String time,
    ...
    Schedule schedule = scheduleRepository.findByMessage(message)
        .orElseGet(() -> new Schedule(message, guildRegistration));
